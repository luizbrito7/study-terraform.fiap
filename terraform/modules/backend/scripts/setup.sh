...
if gh variable list --repo "${GITHUB_REPOSITORY}" | grep -q "AWS_BUCKET_TFSTATE_ID"; then
    ...
    # random name
	BUCKET_NAME="bucket-tfstate-${GITHUB_RUN_ID}"
	log_info "Gerando nome do bucket: $BUCKET_NAME"
	...
	terraform -chdir=./terraform/modules/backend plan -var="bucket_tfstate=$BUCKET_NAME" -out tfplan
	...
	BUCKET_NAME_OUTPUT=$(terraform -chdir=./terraform/modules/backend output -raw bucket_name)
	check_exit_code $? "Terraform output"
	
	log_info "Criando repository variable..."
	gh variable set AWS_BUCKET_TFSTATE_ID --repo "${GITHUB_REPOSITORY}" --body "$BUCKET_NAME_OUTPUT"
	check_exit_code $? "Repository variable creation"
	
	echo "AWS_BUCKET_TFSTATE_ID=$BUCKET_NAME_OUTPUT" >> $GITHUB_OUTPUT
	log_success "Backend setup completo! Bucket: $BUCKET_NAME_OUTPUT"
fi
